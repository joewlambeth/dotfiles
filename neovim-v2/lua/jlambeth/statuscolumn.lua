...
function _G.STATUS_LINE()
	...
	local autoformat_on = conform_toggle() == "ON" and "" or "󰜺"
	local autoformat_highlight = conform_toggle() == "ON" and "%#StAFYes#" or "%#StAFNo#"

	if vim.bo.readonly then
		autoformat_on = "Read Only"
		autoformat_highlight = "%#StAFNo#"
	end
	...
	local highlights = {
		...
		"%#StType#",
		"%#StMid#",
		autoformat_highlight,
		-- "%#StPos# %l,%c",
	}
	if focused then
		for i, s in ipairs(data) do
			data[i] = highlights[i] .. data[i]
		end
		return table.concat(data) .. "%#StatusLine#"
	else
